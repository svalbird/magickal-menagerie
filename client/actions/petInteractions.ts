...
import { getPetInteraction, updatePet, deleteInventoryItem } from "../apis/petInteractions";
...
export function setPetInteractionSuccess(data: PetInteractionData): PetInteractionAction {
  return {
...
export function fetchPetInteraction(): ThunkAction {
  return async (dispatch) => {
    try {
      ...
      // Handle error case
      console.error('Error fetching pet interaction data or updating inventory:', error);}
    }
  };
}
...
export function updatePetInteractionAndDeleteInventoryItem(pet: PetData, itemId: number): ThunkAction {
  return async (dispatch) => {
    try {
      await updatePet(pet);
      ...
      // Handle success case[]
      console.log('Pet interaction data updated and inventory item deleted successfully!');
    } catch (error) {
      if (error instanceof Error){
      dispatch(setError(error.message));
      // Handle error case
      console.error('Error updating pet interaction data or deleting inventory item:', error);
    }
   }
  };
}
